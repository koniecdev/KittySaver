...
public class AuthApiHttpClient(
    HttpClient client,
    ICurrentUserService currentUserService) : IAuthApiHttpClient
{
    ...
    public async Task<ICollection<IAuthApiHttpClient.PersonDto>> GetPersons()
    {
        await SetUpAuthorization();
        HttpResponseMessage response = await client.GetAsync("v1/persons");
        ICollection<IAuthApiHttpClient.PersonDto>? dtos = await response.Content.ReadFromJsonAsync<ICollection<IAuthApiHttpClient.PersonDto>>();
        dtos ??= new List<IAuthApiHttpClient.PersonDto>();
        return dtos;
    }
    
    public async Task<IAuthApiHttpClient.PersonDto> GetPerson(Guid id)
    {
        await SetUpAuthorization();
        HttpResponseMessage response = await client.GetAsync($"v1/persons/{id}");
        IAuthApiHttpClient.PersonDto dto = await response.Content.ReadFromJsonAsync<IAuthApiHttpClient.PersonDto>()
    ...
    public async Task UpdatePerson(Guid id, IAuthApiHttpClient.UpdatePersonDto request)
    {
        await SetUpAuthorization();
        string serializedRequest = JsonSerializer.Serialize(request);
        StringContent bodyContent = new StringContent(serializedRequest, Encoding.UTF8, "application/json");
